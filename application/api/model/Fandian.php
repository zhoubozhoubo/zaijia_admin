        ...
        if ($vid != 0) {
            ...
            /*if ($vid != $sid && $vid != $fid) {
                $vfandian = array(
                    ...
                    'username' => Db::name('User')->where('id', $this->id)->value('username'),
                    'tid' => $vid,
                    'tusername' => Db::name('User')->where('id', $vid)->value('username'),
                    'status'=>2,
                    'type'=>10,//育成团队返点
                    'money' => $this->money * 0.01,
                    'time' => date('Y-m-d H:i:s'),
                );
                //添加佣金返点记录数据
                Db::name('Sale')->insert($vfandian);
                //团队连创人加钱
                $vmoney = Db::name('User')->where('id', $vid)->value('money');
                Db::name('User')->where('id', $vid)->update(['money' => $vmoney + $this->money * 0.01]);
