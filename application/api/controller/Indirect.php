...
class Indirect extends Base
{
    ...
    public function index()
    {
        ...
        //记录列表
        $list = Db::name('User')->where(['sid' =>$id])->where('exp','neq',1)->page($post['page'], $post['limit'])->order('time desc')->select();
        foreach ($list as $k=>$v) {
            //总货款
            $all_money= Db::name('Sale')->where(['uid'=>$v['id'],'type'=>2])->sum('money');
            $list[$k]['all_money']=number_format($all_money,2);
            //总提成
            $ti_money= Db::name('Sale')->where(['uid'=>$v['id'],'tid'=>$id])->where('type','7|8|10')->sum('money');
            $list[$k]['ti_money']=number_format($ti_money,2);
        }
        $count=Db::name('User')->where(['sid' => $id])->where('exp','neq',1)->count();
        if(!isset($count)){
            $count=0;
        }
        //记录总页数
